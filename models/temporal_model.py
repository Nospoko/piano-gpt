...
from models.model import Block, LayerNorm


@dataclass
class TempoGPTConfig:
    block_size: int = 1024
...
class TempoGPT(nn.Module):
    def __init__(
        ...
        self.transformer = nn.ModuleDict(
            dict(
                ...
                wtime=nn.Embedding(max_time_steps, config.n_embd),
    ...
    def forward(self, idx, targets=None, target_mask=None, time_steps=None):
        ...
        if time_steps is not None:
            ...
            time_emb = self.transformer.wtime(time_steps)  # (b, t, n_embd)
    ...
    def from_pretrained(cls, model_type, override_args=None):
        ...
        config = TempoGPTConfig(**config_args)
        model = TempoGPT(config)
        sd = model.state_dict()
        sd_keys = sd.keys()
