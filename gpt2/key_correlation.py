...
class SpiralPoint:
    ...
    def __add__(self, other: 'SpiralPoint') -> 'SpiralPoint':
        return SpiralPoint(
            self.x + other.x,
            self.y + other.y,
            self.z + other.z
    ...
    def __mul__(self, scalar: float) -> 'SpiralPoint':
        return SpiralPoint(
            self.x * scalar,
            self.y * scalar,
            self.z * scalar
    ...
    def distance(self, other: 'SpiralPoint') -> float:
        return math.sqrt(
            (self.x - other.x) ** 2 +
            (self.y - other.y) ** 2 +
            (self.z - other.z) ** 2
...
class SpiralArray:
    ...
    def _generate_major_keys(self) -> List[SpiralPoint]:
        ...
        for root in range(12):
            ...
            key_point = (
                self.pitch_classes[root] * 0.4 +
                self.pitch_classes[third] * 0.3 +
                self.pitch_classes[fifth] * 0.3
            )
    ...
    def get_center_of_effect(self, 
                           pitches: List[int], 
                           durations: List[float],
                           velocities: List[float]) -> SpiralPoint:
        """
    ...
    def get_key(self, 
                center: SpiralPoint, 
                return_distribution: bool = False) -> Union[int, Tuple[int, np.ndarray]]:
        """
...
def calculate_key_correlation(
    ...
    def get_key_distribution(segments: List[pd.DataFrame], spiral: SpiralArray) -> np.ndarray:
        ...
        for segment_df in segments:
            ...
            if use_weighted:
                ...
                durations = [
                    min(row['end'], segment_duration) - max(row['start'], 0)
    ...
    metrics = {
        ...
        "target_top_keys": [
            spiral.key_names[i] 
            for i in np.argsort(-target_dist)[:3]
        ],
        "generated_top_keys": [
            spiral.key_names[i] 
            for i in np.argsort(-generated_dist)[:3]
        ]
    }
